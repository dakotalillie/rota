...
test.describe("worker", () => {
  test.use({
    seedFile: path.join(__dirname, "../seed/rotation-handoff-pending.json"),
  });
  ...
  test("worker advances rotation at handoff time", async ({
    ...
    // eslint-disable-next-line playwright/no-wait-for-timeout -- waiting on backend worker tick, no UI signal to await
